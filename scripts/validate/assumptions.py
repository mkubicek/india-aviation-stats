...
1. Re-run each file's named ``falsification`` test against current data →
   ...
   → advisory).
2. **Reverse gate (completeness by construction):** a concurrent same-canonical
   merge with *no* covering assumption file is an undocumented quirk → BLOCKING.
   Scope is exactly that: it does not re-derive the other mechanical findings, so
   a high-volume unmapped name is caught by its own advisory in ``overlap.py``,
   not here.
"""
